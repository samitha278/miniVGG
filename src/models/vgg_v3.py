...
class MLP(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.layer = nn.Linear(config.n_embd,2*config.n_embd)
        self.relu = nn.ReLU()
        self.proj = nn.Linear(2*config.n_embd,config.n_class)
